...
cutoff <- as.Date(paste(report_year,"7","06",sep="-"))
cutoff2 <- as.Date(paste(report_year,"6","24",sep="-"))
...
if(nrow(ow_microcystin) != 0){
    ...
    annotate(geom="label", x=cutoff2, y=2, label="Below NYSDOH Recreation Level of Concern",
              color="black", alpha=0.9) +
    annotate(geom="label", x=cutoff, y=20, label="Openwater NYSDEC Confirmed With High Toxins Bloom",
              color="black", alpha=0.9) +
    annotate("segment", x = arrow_loc, xend = arrow_loc, y = 16, yend = 70, arrow=arrow()) +
    annotate("segment", x = arrow_loc, xend = arrow_loc, y = 2.5, yend = 0.5, arrow=arrow()) +
    theme_classic() +
...
cutoff <- as.Date(paste(report_year,"7","04",sep="-"))
cutoff2 <- as.Date(paste(report_year,"6","24",sep="-"))
arrow_loc <-as.Date(paste(report_year,"5","1",sep="-"))
...
if(nrow(s_microcystin) != 0){
  ...
  annotate(geom="label", x=cutoff2, y=2, label="Below NYSDOH Recreation Level of Concern",
              color="black", alpha=0.9) +
  annotate(geom="label", x=cutoff, y=30, label="Shoreline NYSDEC Confirmed With High Toxins Bloom",
              color="black", alpha=0.9) +
  annotate("segment", x = arrow_loc, xend = arrow_loc, y = 24, yend = 90, arrow=arrow()) +
  annotate("segment", x = arrow_loc, xend = arrow_loc, y = 2.5, yend = 0.5, arrow=arrow()) +
  theme_classic() +
  scale_x_continuous(limits=c(may1,nov1),
                     breaks=c(may1,june1,july1,aug1,sept1,oct1, nov1), 
