...
for (i in 1:(nrow(z_score_df))) {
  if(is.na(z_score_df$z_score[i])){
    ...
    z_score_df$diff[i] <- "no"
  }
  else if(z_score_df$z_score[i] >= 1 & z_score_df$z_score[i] < 2){
    z_score_df$diff[i] <- "one_up_arrow"
  }
  else if(z_score_df$z_score[i] >= 2){
    z_score_df$diff[i] <- "two_up_arrow"
  }
  else if(z_score_df$z_score[i] <= -1 & z_score_df$z_score[i] > -2){
    z_score_df$diff[i] <- "one_down_arrow"
  }
  else if(z_score_df$z_score[i] >= -2){
    z_score_df$diff[i] <- "two_down_arrow"
  }
}
...
owi_withplots2 <- left_join(owi_withplots1, z_score_df) %>% 
  ungroup()
...
make_table <- owi_withplots %>% 
  ...
  tab_options(column_labels.background.color = "grey") %>% 
   text_transform(
    locations = cells_body(vars(diff)),
    fn = function(x) {
